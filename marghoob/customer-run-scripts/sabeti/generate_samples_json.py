...
raw_path="/net/kodiak/volumes/delta/shared/prj/sabeti/raw/"
...
for line in sys.stdin.readlines():
  ...
  for bam in glob('%s/*.rg.bam' % (sample_path)):
    ...
    fields = bam.split(".")
    bam_rg = os.path.splitext(os.path.splitext(os.path.split(bam)[1])[0])[0].replace(sample_path_split[-1] + ".", "", 1)
    ...
    for readgroup in readgroups:
      if readgroup['ID'] == bam_rg:
        ...
        sys.stderr.write("Readgroup %s matched\n" % (readgroup))
  ...
  if skip: continue
  sample_path_split = sample_path.split("/")
  output_prefix = "/net/kodiak/volumes/delta/shared/prj/sabeti/runs/%s" % (sample_path_split[-1])
  if not os.path.exists(output_prefix):
    sys.stderr.write("%s non-existent\n" % (output_prefix))
    os.makedirs(output_prefix)
  dataset = {"alignment_groups": alignment_readgroups, "output_prefix": "delta:/%s" % (os.path.relpath(output_prefix, delta_path))}
  datasets[sample_path_split[-1]] = dataset
