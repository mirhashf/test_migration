...
is_na12878_platinum_50x = (re.match(".*/ERR194147_.*", workflow_json["alignment_groups"][0]["fastq"][0]) is not None)
is_na12878_platinum_downsampled_30x = (re.match(".*/ERR194147_downsampled.*", workflow_json["alignment_groups"][0]["fastq"][0]) is not None)
is_na12878_wes = (re.match(".*garvan_data.*", workflow_json["alignment_groups"][0]["fastq"][0]) is not None)

dataset_name = "NA12878_platinum_50x" if (is_na12878_platinum_50x and not is_na12878_platinum_downsampled_30x) else ("NA12878_platinum_downsampled_30x" if is_na12878_platinum_downsampled_30x else ("NA12878_garvan_100x" if is_na12878_wes else "unknown"))

print worker_num, thread_num, time_taken, has_avx, gatk_version, ("desktop" if is_desktop else "enterprise"), aligner, data_type, dataset_name
