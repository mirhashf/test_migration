...
vcf-concat deletions.hg19.vcf.gz INDEL.vcf.gz | vcf-sort -c | bgzip > SV.INDEL.vcf.gz && tabix -f SV.INDEL.vcf.gz

for vcf in deletions.hg19.vcf.gz INDEL.vcf.gz SNP.vcf.gz; do
  (
...
cd $GENOMEDIR
java -jar $VCF2DIPLOID -id NA12878 -chr $REFERENCE -vcf $VCFDIR/deletions.hg19.vcf $VCFDIR/INDEL.vcf $VCFDIR/SNP.vcf &>$LOGDIR/vcf2diploid.log
